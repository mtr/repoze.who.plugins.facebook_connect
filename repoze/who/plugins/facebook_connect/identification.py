    ...
    def identify(self, environ):
        ...
        if not fb_user:
            environ[REPOZE_WHO_LOGGER] \
                .warn('Facebook Python-SDK did not return any user data.')
            self._logout_and_redirect(environ)
        ...
        environ[REPOZE_WHO_LOGGER] \
            .warn('Received (from cookie) fb_user = %s', fb_user)
        ...
        try:
            ...
            if not ('id' in profile and profile['id'] == fb_user['uid']):
                environ[REPOZE_WHO_LOGGER] \
                    .warn('Facebook Python-SDK received invalid uid.')
                self._logout_and_redirect(environ)
                return None
        ...
        authenticated, redirect_to_url, cookies \
            = self.identified_hook(fb_user['uid'], profile, environ, identity)
